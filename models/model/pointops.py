...
def _load_torch_shared_libs() -> None:
    """Preload PyTorch shared libraries for custom CUDA extensions."""
...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        idx = torch.zeros(
            new_offset[b-1].item(), device=xyz.device, dtype=torch.int32
        )
        tmp = torch.full((n,), 1e10, device=xyz.device, dtype=xyz.dtype)
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        dist2 = torch.zeros((m, nsample), device=xyz.device, dtype=xyz.dtype)
...
class Grouping(Function):
    ...
    def forward(ctx, input, idx):
        ...
        output = torch.empty((m, nsample, c), device=input.device, dtype=input.dtype)
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        output = torch.zeros((n, nsample, c), device=input1.device, dtype=input1.dtype)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros((n, c), device=grad_output.device, dtype=grad_output.dtype)
        grad_input2 = torch.zeros((n, c), device=grad_output.device, dtype=grad_output.dtype)
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        output = torch.zeros((n, c), device=input.device, dtype=input.dtype)
    ...
    def backward(ctx, grad_output):
        ...
        n, nsample, c = position.shape; w_c = weight.shape[-1]
        grad_input = torch.zeros((n, c), device=grad_output.device, dtype=grad_output.dtype)
        grad_position = torch.zeros((n, nsample, c), device=grad_output.device, dtype=grad_output.dtype)
        grad_weight = torch.zeros((n, nsample, w_c), device=grad_output.device, dtype=grad_output.dtype)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    new_feat = torch.zeros(
        (new_xyz.shape[0], feat.shape[1]), device=feat.device, dtype=feat.dtype
    )
    for i in range(k):
