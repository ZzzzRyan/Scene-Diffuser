...
@OPTIMIZER.register()
class MotionInSceneOptimizer(Optimizer):
    ...
    def __init__(self, cfg: DictConfig, slurm: bool, *args, **kwargs) -> None:
        ...
        if self.contact or self.collision or self.smoothness:
    ...
    def optimize(self, x: torch.Tensor, data: Dict) -> torch.Tensor:
        ...
        if self.contact:
            B, T, V, D = vertices.shape
            loss_contact = 0.
            for t in range(1, T, self.frame_interval): # start is given
                ...
                loss_contact += cham_dist
            
            loss += self.contact_weight * loss_contact
        ...
        if self.collision:
            ...
            loss_collision = 0.
            for t in range(1, T, self.frame_interval): # start is given
                vertices_trans = transform_verts(vertices[:, t, :, :], scene_trans_inv)

                s_grid_min_ = s_grid_min.unsqueeze(1)
                s_grid_max_ = s_grid_max.unsqueeze(1)

                norm_verts_batch = ((vertices_trans - s_grid_min_) 
                                        / (s_grid_max_ - s_grid_min_) * 2 -1)
                ...
                loss_collision += sdf_pene
            
            loss += self.collision_weight * loss_collision
        
        if self.smoothness:
            velocity = vertices[:, 1:, :, :] - vertices[:, 0:-1, :, :]
            acceleration = velocity[:, 1:, :, :] - velocity[:, 0:-1, :, :]
            loss_smoothness = (acceleration ** 2).mean()

            loss += self.smoothness_weight * loss_smoothness

        ## TODO add foot location constraints to alleviate foot skating
