...
def main(cfg: DictConfig) -> None:
    ...
    res_dir = os.path.join(eval_dir, 'metrics', timestamp_str())
    
    logger.add(res_dir + '/test.log') # set logger file
    ...
    ## prepare dataset for quantitative evaluation
    ## only load scene
    datasets = {
    ...
    ## create model and diffuser, and load cpkt
    ...
    ## ceate evaluator and evaluate
    evaluator = create_evaluator(cfg.task.evaluator)
    evaluator.evaluate(model, dataloaders['test'], res_dir)
